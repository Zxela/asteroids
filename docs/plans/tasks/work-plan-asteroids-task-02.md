...
- [x] `src/ecs/EntityManager.ts` - Entity creation/destruction, ID allocation
- [x] `src/ecs/ComponentStorage.ts` - Component archetype storage and querying
- [x] `src/ecs/SystemManager.ts` - System registration and update orchestration
- [x] `src/ecs/World.ts` - Public interface for entity/component/system operations
- [x] `src/ecs/index.ts` - Public API exports
- [x] `tests/unit/ecs.test.ts` - Unit tests (Red state)
...
- [x] Entity creation returns unique EntityId
- [x] Entity destruction removes entity from system
- [x] Component add/remove works per entity
- [x] Component queries return correct entities
- [x] System registration and execution in order
- [x] World.update() orchestrates all systems
- [x] Query with multiple component requirements works
- [x] Destroying entity removes all components
- [x] Systems can be removed

### 2. Green Phase

**Implement minimal code to pass tests**:

- [x] Implement `src/ecs/types.ts`:
  ```typescript
...
- [x] Implement `ComponentStorage.ts`:
...
- [x] Export public API from `src/ecs/index.ts`:
  ```typescript
...
### 3. Refactor Phase
- [x] Verify no unnecessary allocations in hot path (query, update)
- [x] Ensure type safety throughout (no `any` types)
- [x] Document complex methods with JSDoc
- [x] Optimize ComponentStorage for memory locality
- [x] Ensure all tests still pass

## Completion Criteria

- [x] ECS World can create and destroy entities
- [x] Components can be added, removed, and queried from entities
- [x] Systems can be registered and updated in order
- [x] Unit tests created and all passing (5+ test suites)
- [x] TypeScript strict mode passes
- [x] No circular dependencies
- [x] Query performance acceptable (<1ms for 100 entities)
